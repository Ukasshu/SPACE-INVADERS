...
var
  ...
  //plnum   : integer=2;
  mode       :  integer = 1;
  tex        :  array [0..23] of zglPTexture; {0 - tlo, 1 - title, 2 - digits, 3 - heart, 4-pause}
...
function ckif(en:enemy; pl:player):boolean;
var
  i:integer;
  a,b,c : real;
begin
  ...
  begin
    a:=sin(i*pi/(en.fp+1));
    b:=-cos(i*pi/(en.fp+1));
    c:=-a*(en.posx+en.w/2)-b*(en.posy+en.h);
    if ((a*pl.posx+b*pl.posy+c)*(a*(pl.posx+pl.w)+b*(pl.posy+pl.h)+c)<=0) OR ((a*pl.posx+b*(pl.posy+pl.h)+c)*(a*(pl.posx+pl.w)+b*pl.posy+c)<=0) then
      ckif:=true
    else if i=en.fp then
      ckif:=false;
  end;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
             tempe:=spre^;
             if (tempe=upgr.who) AND not(upgr.ismoving) then
...
begin
  ...
  tex[23] := tex_LoadFromFile('textures/htp.png');
end;
  ...
  begin
    ...
    if gracz.HP>0 then
      ...
      changemode(6);
       ...
       changemode(3);
  if gracz.score>=lastscore+300 then
  begin
    lastscore:=lastscore+300;
    gracz.HP:=gracz.HP+1;
    snd_play(dzw[6]);
  end;
end;
...
begin
  ...
  begin
    ...
    ssprite2d_Draw(tex[23],0,0,1280,720,0);
end;
